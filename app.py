import io, os, re, base64, shutil, platform, json
import numpy as np
import pandas as pd
...
# =========================
# iPhone numeric keypad input
# =========================
def ios_number_input(label: str, key: str, value: str = "", allow_decimal: bool = True, placeholder: str = "") -> str:
    """
    Streamlit text_input that forces iPhone's native numeric keypad by setting inputmode on this exact field.
    Keeps Streamlit state/keys intact. Returns a cleaned string.
    """
...
def render_logo(max_width=350):
    ...
    logo_path = next((os.path.join(base_dir, n) for n in candidates if os.path.isfile(os.path.join(base_dir, n))), None)
    if not logo_path:
...
def pretty_cbr_plot(settle, pressure, bp125=None, compact=False):
    ...
    if bp125 is not None:
        fig.add_shape(type="line", x0=1.25, x1=1.25, y0=0, y1=y_max, line=dict(color="rgba(255,0,0,0.35)", width=2, dash="dash"))
...
def render_cbr_banner(cbr_pct: float, layer: str):
    ...
    st.markdown(
        f"""
        <div class="awk-banner" style="padding:14px 16px; border:1px solid {border}; background:{bg};
        border-radius:12px; color:{text};">
            <div style="font-size:14px; font-weight:600; letter-spacing:0.2px;">Equivalent In-Situ CBR</div>
            <div style="font-size:30px; font-weight:800; line-height:1.1; margin-top:4px;">{rounded_value:.1f}%</div>
            <div style="font-size:13px; margin-top:6px;">Layer: {layer} &nbsp;&nbsp; {status_line}</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
# =========================
# Manual Entry (with iPhone keypad)
# =========================
if method == "Manual entry":
    ...
    for L in LOAD_STEPS:
        ...
        c0.write(f"{L:.0f}")
        with c1:
            d1 = ios_number_input(f"Dial 1 (mm) – {L} kN", key=f"d1_{L}", allow_decimal=True, placeholder="")
        with c2:
            d2 = ios_number_input(f"Dial 2 (mm) – {L} kN", key=f"d2_{L}", allow_decimal=True, placeholder="")
        with c3:
            d3 = ios_number_input(f"Dial 3 (mm) – {L} kN", key=f"d3_{L}", allow_decimal=True, placeholder="")
    ...
    for L in LOAD_STEPS:
        ...
        d1f, d2f, d3f = to_float_or_none(d1s), to_float_or_none(d2s), to_float_or_none(d3s)
